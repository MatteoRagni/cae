...
class ConvAutoEncSettings(object):
  ...
  def checkComplete(self):
    if not(self.input_shape      != None and
           self.corruption       != None and
           self.layers           != None and
           self.patch_size       != None and
           self.strides          != None and
           self.padding          != None and
           self.cuda_enabled     != None and
           self.depth_increasing != None):
      self._inspect()
      raise ArgumentError("Some of the element are not defined")
    if self.corruption == True:
      if not(self.corruption_min != None and self.corruption_max != None):
        self._inspect()
        raise ArgumentError("You request corruption limits but corruption is not defined")
    return self
  ...
  def corruption_min(self, value):
    ...
    if self.corruption_max:
      if value > self.corruption_max:
       raise ArgumentError("corruption_min ({}) is greater than corruption_max ({})".format(self.corruption_min, self.corruption_max))
  ...
  def layers(self, value):
    if value:
      ...
      if value < 1:
        raise ArgumentLayer("Number of layers must be positive")
    self.__layers = value
  ...
  def padding(self, value):
    if value:
      ...
      if value != "SAME" and value != "VALID":
        raise ArgumentError("Padding can be SAME or VALID. Received {}".format(value))
  ...
  def _checkList(self, obj, size = 0):
    ...
    self._checkInt(size)
    if size > 0:
      assert len(obj) == size, "Size error for list: %d != %d" % (len(obj), size)
    ...
    if keylist:
      ...
      for k in keylist:
        try:
          ...
          raise KeyError("Key %r is not defined in object %r" % (k, obj))
...
class ConvAutoEnc(object):
  def __init__(self, settings):
    assert type(settings) is ConvAutoEncSettings, "Settings are not of type ConvAutoSettings"
    ...
    with self.graph.as_default():
      with tf.name_scope("encoder"):
        ...
        for layer in range(0, self.layers):
          ...
          patch = [self.patch_size, self.patch_size, old_depth, new_depth]
          ...
          W = tf.Variable(tf.truncated_normal(patch, stddev=0.1), name=name_W)
          ...
          h = tf.add(
              tf.nn.conv2d(x_current, W, strides=self.strides, padding=self.padding, name=name_conv) ,
              B, name=name_sum
            )
      ...
      with tf.name_scope("decoder"):
        ...
        for layer in range(0, self.layers):
          ...
          h = self.leakRelu(tf.add(
              tf.nn.conv2d_traspose(x_current, W, shape, strides=self.strides, padding=self.padding, name=name_deconv),
              B, name=name_sum
          ))
          x_current = self.leakRelu(h, name=name_out)
        self.y = x_current
  ...
  def defineCost():
    with self.graph.as_default():
      with tf.name_scope("cost-function"):
        self.error = tf.reduce_sum(tf.square(self.y - self.x), name="error-definition")
